...
class ParallelConfig:
    ...
    - "nccl" (default): two ``dist.all_to_all_single`` calls for
      output and LSE, then a Triton LSE-combine.
    - "flashinfer": a single fused FlashInfer ``decode_cp_a2a_alltoall``
      kernel (LL128 + MNNVL when multi-node), then the same Triton
      LSE-combine. Requires ``flashinfer`` installed and
